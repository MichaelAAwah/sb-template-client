...
import { getCurrencySymbol, convertPrice } from '../../api/salesData';
...
export const SalesOrderItemsTable: React.FC<SalesOrderItemsTableProps> = ({
  ...
  const handleItemSelect = (index: number, itemId: string) => {
    ...
    if (selectedItem) {
      // Convert unit price from GHS to selected currency
      const convertedPrice = convertPrice(selectedItem.unitPrice, currency);
      ...
      updateItem(index, 'unitPrice', convertedPrice);
      updateItem(index, 'uomName', selectedItem.uomName);
      updateItem(index, 'location', selectedItem.location);
      // Recalculate total
      const newItems = [...items];
      newItems[index].total = newItems[index].quantity * convertedPrice;
      onItemsChange(newItems);
    }
  };
                ...
                <TableHeader className="sticky top-0 bg-white z-10">
                  <TableRow>
                    ...
                    {showOriginalQuantity && (
                      <TableHead className="w-24">SQ Qty</TableHead>
                    )}
                    <TableHead className="w-24">Qty</TableHead>
                    <TableHead className="w-28">Unit Price</TableHead>
                    <TableHead className="w-28">Total ({currency})</TableHead>
                ...
                <TableBody>
                  {items.map((item, index) => (
                    <TableRow key={item.id}>
                      ...
                      {showOriginalQuantity && (
                        <TableCell>
                          <div className="h-8 flex items-center text-sm text-gray-600 bg-gray-50 px-2 rounded">
                  ...
                  {items.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={showOriginalQuantity ? 15 : 14} className="text-center py-8 text-gray-500">
